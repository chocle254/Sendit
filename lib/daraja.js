...
// Safaricom ResultCodes that mean "the user themselves backed out" rather
// than a generic failure — surfaced as a distinct 'cancelled' status instead
// of being lumped into 'failed'.
// 1032 = "Request cancelled by user" (pressed cancel on the STK prompt)
// 1037 = "DS timeout user cannot be reached" (didn't respond in time)
const CANCELLED_RESULT_CODES = new Set([1032, 1037]);

// Classifies a Safaricom ResultCode into 'success' | 'cancelled' | 'failed'.
...
// Maps a stkPushQuery response to 'success' | 'cancelled' | 'failed' | 'pending'.
// Never throws — an ambiguous/error-shaped response is reported as pending so
// the caller just polls again rather than mistaking a transient state for a
// real failure.
export function mapQueryStatus(data) {
  ...
  return classifyResultCode(code);
}
...
export function parseStkCallback(body) {
  ...
  if (!cb) return null;
  const status = classifyResultCode(Number(cb.ResultCode));
  const result = {
    ...
    success: status === "success", // kept for existing callers that only check success/failure
    mpesaReceipt: null,
    amount: null,
    phone: null,
  };
