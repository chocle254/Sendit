...
export const MIN_TOKENS_PURCHASE = 10; // lowered from 50 for testing — raise back before real launch
...
// Prices lowered to 10 for testing — raise back to real prices before launch.
export const PLAN_PRICES_KES = { monthly: 10, yearly: 10 };
...
export async function createTrialAccount({ userId, businessName, accountType, payoutPhone, tillNumber, paybillNumber, paybillAccountNumber, detailsConfirmed }) {
  ...
  // details_confirmed_at is the paper trail for the "I confirm these
  // details are correct" acknowledgment — link.js already rejects the
  // request server-side if detailsConfirmed !== true, so by the time we
  // get here it's always meant to be set. Timestamp is generated on the
  // server (new Date()), never taken from the client, so it can't be
  // backdated or forged.
  const confirmedAt = detailsConfirmed ? new Date() : null;
  const { rows } = await sql`
...
// --- Time-bucketed series for the admin & developer line graphs ----------

// Whitelisted rather than passed straight through: `trunc` gets spliced into
// date_trunc(), which Postgres won't take as a bound param there — so only
// ever accept one of these four exact keys, never a raw query-string value.
// `lookback` IS safely bindable as a normal ::interval parameter below.
const GRANULARITY_CONFIG = {
  hourly: { trunc: "hour", lookback: "48 hours", labelFmt: "YYYY-MM-DD HH24:00" },
  daily: { trunc: "day", lookback: "30 days", labelFmt: "YYYY-MM-DD" },
  monthly: { trunc: "month", lookback: "12 months", labelFmt: "YYYY-MM" },
  yearly: { trunc: "year", lookback: "5 years", labelFmt: "YYYY" },
};

function resolveGranularity(granularity) {
  return GRANULARITY_CONFIG[granularity] || GRANULARITY_CONFIG.daily;
}

// Platform-wide revenue series (tokens, subscriptions, activation fees, and
// parole-specific token purchases) plus money pushed through developers'
// STK pushes — bucketed by the requested granularity. Powers the admin
// dashboard's line graph with an hourly/daily/monthly/yearly toggle.
//
// "Parole tokens" aren't their own transaction type — they're ordinary
// token_purchase transactions made while an account is on_parole. That
// context isn't stamped onto the transaction row itself, but token_ledger
// does record *why* each token delta happened ('parole_penalty' debits vs
// 'purchase' credits), so a purchase is counted as parole-driven here when
// it falls within a day of a parole-penalty debit on the same account. This
// is a heuristic based on timing, not a stored fact — good enough for a
// trend line, not for per-transaction attribution.
export async function getAdminRevenueSeries(granularity) {
  const { trunc, lookback, labelFmt } = resolveGranularity(granularity);

  const { rows: series } = await sql`
    SELECT
      to_char(date_trunc(${trunc}, created_at), ${labelFmt}) AS bucket,
      COALESCE(SUM(amount) FILTER (WHERE type = 'token_purchase' AND status = 'success'), 0) AS token_revenue,
      COALESCE(SUM(amount) FILTER (WHERE type = 'subscription' AND status = 'success'), 0) AS subscription_revenue,
      COALESCE(SUM(amount) FILTER (WHERE type = 'activation' AND status = 'success'), 0) AS activation_revenue,
      COALESCE(SUM(amount) FILTER (WHERE type = 'stkpush' AND status = 'success'), 0) AS money_pushed
    FROM transactions
  ...
  const { rows: paroleRows } = await sql`
    SELECT
      to_char(date_trunc(${trunc}, tl_purchase.created_at), ${labelFmt}) AS bucket,
      COALESCE(SUM(t.amount), 0) AS parole_token_revenue
    FROM token_ledger tl_purchase
    JOIN transactions t ON t.id = tl_purchase.related_transaction_id
    WHERE tl_purchase.reason = 'purchase'
      AND tl_purchase.created_at >= now() - ${lookback}::interval
      AND EXISTS (
        SELECT 1 FROM token_ledger tl_penalty
        WHERE tl_penalty.account_id = tl_purchase.account_id
          AND tl_penalty.reason = 'parole_penalty'
          AND tl_penalty.created_at <= tl_purchase.created_at
          AND tl_penalty.created_at >= tl_purchase.created_at - interval '1 day'
      )
    GROUP BY 1
    ORDER BY 1
  `;

  const paroleByBucket = new Map(paroleRows.map((r) => [r.bucket, Number(r.parole_token_revenue)]));

  return series.map((r) => ({
    bucket: r.bucket,
    tokenRevenue: Number(r.token_revenue),
    subscriptionRevenue: Number(r.subscription_revenue),
    activationRevenue: Number(r.activation_revenue),
    paroleTokenRevenue: paroleByBucket.get(r.bucket) || 0,
    totalRevenue: Number(r.token_revenue) + Number(r.subscription_revenue) + Number(r.activation_revenue),
    moneyPushed: Number(r.money_pushed),
  }));
}

// Per-developer series: how much money this user's linked accounts pushed
// via STK push, bucketed by granularity. Powers the "amount pushed through
// your STK pushes" line graph on the regular developer dashboard.
export async function getUserStkPushSeries(userId, granularity) {
  const { trunc, lookback, labelFmt } = resolveGranularity(granularity);

  const { rows } = await sql`
    SELECT
      to_char(date_trunc(${trunc}, t.created_at), ${labelFmt}) AS bucket,
      COALESCE(SUM(t.amount) FILTER (WHERE t.status = 'success'), 0) AS amount_pushed,
    ...
    WHERE a.user_id = ${userId}
      AND t.type = 'stkpush'
      AND t.created_at >= now() - ${lookback}::interval
    GROUP BY 1
    ORDER BY 1
  `;
  ...
  return rows.map((r) => ({
    bucket: r.bucket,
    amountPushed: Number(r.amount_pushed),
    successfulCount: Number(r.successful_count),
