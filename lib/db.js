...
export async function createTrialAccount({ userId, businessName, accountType, tillNumber, paybillNumber, paybillAccountNumber }) {
  const callbackToken = crypto.randomBytes(20).toString("hex");
  const apiKey = generateApiKeyValue();
  const { rows } = await sql`
    INSERT INTO accounts (
      user_id, business_name, account_type, till_number, paybill_number, paybill_account_number,
      api_key, status, free_tx_used, token_balance, consecutive_failures, on_parole, callback_token
    )
    VALUES (
      ${userId}, ${businessName}, ${accountType}, ${tillNumber || null}, ${paybillNumber || null}, ${paybillAccountNumber || null},
      ${apiKey}, 'trial', 0, 0, 0, FALSE, ${callbackToken}
    )
    RETURNING *
  `;
  return rows[0];
...
// Resets the free-transaction counter so the account gets another 25 free
// pushes; there's no time-based expiry, only usage-based.
...
export function evaluateAccountUsage(account) {
  ...
  if (account.free_tx_used >= FREE_TX_LIMIT) {
    return { allowed: false, reason: `Free tier (${FREE_TX_LIMIT} transactions) used up. Pay KES 350 to reset and continue.` };
  }
