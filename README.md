...
Provision a Postgres database — the easiest path is the **Vercel Postgres**
(or Neon) integration from your Vercel project's Storage tab. Then run
`schema.sql` against it once (via the provider's SQL editor, or `psql`).

### 2. Environment variables

Copy `.env.example` to `.env.local` and fill in:

- `POSTGRES_URL` — usually set automatically by the Vercel Postgres integration
